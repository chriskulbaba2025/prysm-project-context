...
Exact next action:
Run one dependency-closure preflight that validates the complete preferred readback chain in one pass: linked Railway identity; exact project/environment/worker; public worker health; internal-secret authenticated audit-status lookup without printing the secret; returned Bulldog `clientId`; Railway-injected tenant and S3 configuration presence; exact read-only S3 artifact HEAD/GET for the Bulldog canonical AuditRequest; local Node/package requirements; and Downloads proof write access. Do not require direct PostgreSQL connectivity. Collect and attempt to resolve all no-mutation blockers before returning FAIL. If this chain passes, continue automatically through artifact recovery, classification, independent verification, reconciliation, and Evidence Package readiness. If preflight passes, establish a safe isolated-staging readback path for Bulldog audit `560f5640-9ff3-4a68-881c-56f4284867e4` without rerunning providers, then continue in the same tranche through exact artifact identity/readback, external classification, meaningfully independent second verification, reconciliation, and Evidence Package readiness. Prefer a no-deploy local readback using Railway-injected staging environment and the existing governed artifact-store code. Stop immediately if any required access is missing or if continuation would require a new authorization boundary.
