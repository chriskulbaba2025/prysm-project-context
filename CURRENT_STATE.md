...
Exact next action:
Run one closure tranche that first performs full environment-parity proof, then securely migrates only `DATAFORSEO_LOGIN`, `DATAFORSEO_PASSWORD`, and source `VANTAGE_PAGESPEED_API_KEY` mapped to target `GOOGLE_PAGESPEED_API_KEY` from the historical working Railway service into isolated staging without printing or persisting the values. Redeploy the exact isolated worker, verify DataForSEO and PageSpeed authentication, prove actual Playwright Chromium launch in the deployed container via Railway SSH, and only when every readiness gate passes run exactly one fresh Bulldog audit. Continue automatically through evidence-package recovery, external classification, independent verification, reconciliation, and readiness. Production source/infrastructure remains otherwise frozen. If preflight passes, establish a safe isolated-staging readback path for Bulldog audit `560f5640-9ff3-4a68-881c-56f4284867e4` without rerunning providers, then continue in the same tranche through exact artifact identity/readback, external classification, meaningfully independent second verification, reconciliation, and Evidence Package readiness. Prefer a no-deploy local readback using Railway-injected staging environment and the existing governed artifact-store code. Stop immediately if any required access is missing or if continuation would require a new authorization boundary.
