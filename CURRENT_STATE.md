...
Exact next action:
Do NOT run a fresh Bulldog audit yet. First execute a no-cost readiness-only closure tranche. Prove the exact provider/runtime dependency map, determine secret exportability, validate DataForSEO via a no-cost endpoint, prove the current PageSpeed execution strategy against current code, prove actual Playwright Chromium launch/close in the deployed worker via Railway SSH, verify storage/worker/deployment identity, and produce a complete readiness proof. Only after `PRYSM_PROVIDER_READINESS_PASS` should a separate paid Bulldog audit tranche be created. If preflight passes, establish a safe isolated-staging readback path for Bulldog audit `560f5640-9ff3-4a68-881c-56f4284867e4` without rerunning providers, then continue in the same tranche through exact artifact identity/readback, external classification, meaningfully independent second verification, reconciliation, and Evidence Package readiness. Prefer a no-deploy local readback using Railway-injected staging environment and the existing governed artifact-store code. Stop immediately if any required access is missing or if continuation would require a new authorization boundary.
