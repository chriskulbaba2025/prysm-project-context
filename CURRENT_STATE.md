...
Exact next action:
Do not rerun Bulldog yet. First close provider readiness in isolated staging: securely configure `DATAFORSEO_LOGIN`, `DATAFORSEO_PASSWORD`, and `GOOGLE_PAGESPEED_API_KEY` (optionally `GOOGLE_CRUX_API_KEY`); prove DataForSEO credentials with the provider's zero-cost user-data endpoint; prove PageSpeed key validity without running a full audit; prove the deployed worker has a usable Playwright Chromium runtime; then, only after explicit authorization for the required isolated-staging variable mutation/redeploy and bounded paid provider execution, run one fresh Bulldog evidence audit and repeat the Evidence Package validation. If preflight passes, establish a safe isolated-staging readback path for Bulldog audit `560f5640-9ff3-4a68-881c-56f4284867e4` without rerunning providers, then continue in the same tranche through exact artifact identity/readback, external classification, meaningfully independent second verification, reconciliation, and Evidence Package readiness. Prefer a no-deploy local readback using Railway-injected staging environment and the existing governed artifact-store code. Stop immediately if any required access is missing or if continuation would require a new authorization boundary.
