...
| PDV4 | 2026-08-31 | live production finalization after `narrative_ready` | at least two production-shaped finalization branches escaped: image numerator with unavailable/coerced-zero denominator, and PARTIAL heading assessed-scope wording rejected by finalization guard; complete semicolon-separated error list still requires one read-only lifecycle capture | post-freeze / branch coverage escape | PDV3 Whole-App fixtures did not represent P-B14 (`imagesMissingAlt` available while image denominator unavailable with deep content available) or P-B15 (PARTIAL `VAN-TECH-002` assessed-scope wording through finalization semantic guard) | P-B14/P-B15 permanent assembled finalization regressions; add any further row revealed by the complete live lifecycle reason before PDV4 PASS |
