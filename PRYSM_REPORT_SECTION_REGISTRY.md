...
|---|---|---|---|---|---:|---|
| S01 | Executive Scorecard | Executive Scorecard | PRIMARY 01 | ACTIVE — REAL_REPORT_RENDER REPAIR AUTHORIZED | 63 | Deterministic audit PASS. Real TBK replay failed closed because the 2026-09-01 persisted ScoreSet uses pre-enrichment `contentIdeas` while the current contract introduced enriched rows on 2026-09-02. Diagnosis proved 13/13 rows can be deterministically reconstructed from persisted audit request + decision evidence with zero legacy semantic mismatches and no provider call. Authorized seam: in-memory current-replay normalization in `scripts/replay-report.js` plus bounded regression coverage in `scripts/replay-report-cli.test.js`; no schema/canonical/producer/renderer change. After gates pass, rerender real TBK and stop before HUMAN_REVIEW. |
