...
# Keep Git available when tests intentionally remove the caller's normal PATH
# to prove that Codex is not required for human-owned stages. On Git for
# Windows, git.exe is not under /usr/bin, so a Unix-only PATH makes every
# fixture fail before the actual gate assertion is reached.
GIT_BIN="$(command -v git || true)"
[[ -n "$GIT_BIN" ]] || { echo "PRYSM GATE CONTRACT SETUP FAIL: git is unavailable" >&2; exit 1; }
GIT_BIN_DIR="$(cd "$(dirname "$GIT_BIN")" && pwd)"
...
if run_public "$root" env PATH="$SAFE_PATH" bash "$root/prysm-project-context/tools/prysm/start-prysm-p.sh" P1 >"$root/out" 2>&1 \
...
if run_public "$root" env CODEX_THREAD_ID=test PATH="$root/fakebin:$SAFE_PATH" bash "$root/prysm-project-context/tools/prysm/start-prysm-p.sh" P1 >"$root/out" 2>&1 \
...
if run_public "$root" env PATH="$SAFE_PATH" bash "$root/prysm-project-context/tools/prysm/start-prysm-p.sh" P1 >"$root/out" 2>&1; then
...
if run_public "$root" env PRYSM_DIAGNOSTIC_DIR="$root/diag" PATH="$SAFE_PATH" bash "$root/prysm-project-context/tools/prysm/start-prysm-p.sh" P1 >"$root/out" 2>&1 \
