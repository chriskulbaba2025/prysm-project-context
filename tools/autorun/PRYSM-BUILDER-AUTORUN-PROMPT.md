...
14. preserve/relocate any generated diagnostic or gate evidence outside the application repository and prove no current-run evidence artifact remains untracked;
15. push the repair branch normally;
16. verify local application SHA = remote SHA and ahead/behind = 0/0;
17. re-run `git status --short` and require a clean application tree before audit handoff;
18. update `PRYSM_AUTORUN_STATE.json` and current tranche checkpoint/handoff evidence;
19. commit/push governance memory;
20. verify local governance SHA = remote SHA and 0/0 divergence;
21. re-read remote/current governance state;
22. hand the exact application SHA to the independent Auditor by returning `CONTINUE` with `next_role = Auditor` and checkpoint `AUDIT_REQUIRED`.
...
- `CONTINUE` + `next_role=Auditor` only after a tranche candidate is fully gated/pushed **and the application tree is clean**;
