...
- The bootstrap verifies the autorun control plane, fast-forwards only clean unambiguous repository state, runs the P# autorun regression and permanent gate-contract regression, performs recovery/preflight, then enters the continuous loop. It never pulls/reset-cleans over dirty or locally-ahead recovery state.
- On a clean initial P# candidate, the official deterministic P# gate must PASS before Builder execution.
- Every fresh Codex Builder invocation is a P-scoped transaction. The controller records pre-run application/governance branch, HEAD, status and content fingerprint; marks the transaction `RUNNING`; records exact post-run state as `CODEX_EXITED_UNRECONCILED`; validates scope/control-plane integrity; then marks `RECONCILED` only after result/accounting/routing checks succeed.
- The P-scoped entry anchor and transaction journal live outside both repositories. A restart may accept only the recorded entry lineage, the exact entry fingerprint, the exact latest journaled post-state, or a transaction recorded `RUNNING` when interrupted. Same-branch state alone is not sufficient recovery evidence.
- Current reopened P1 has one special initial adoption path because valid Builder edits predate this controller. That adoption is allowed only on the exact governed P1 branch/base and only when every dirty application path is inside the explicitly authorized report-projection/test seam. The exact adopted fingerprint is then anchored; arbitrary later dirty trees are not re-adopted.
- After every Codex run, committed and uncommitted changed paths are checked against the active P1 bounded seam. Scope escape is rejected before the result is accepted.
- The active Builder may not modify the autorun controller, wrapper, Builder contract, schema, permanent autorun decision, or permanent memory governing the current run. Working-tree and committed control-plane fingerprints are enforced.
...
- Repeated identical Builder continuations with no repository progress trigger a controller anti-thrash stop instead of looping indefinitely.
...
- P# controller accounting, entry anchor, transaction journal, logs, controller state and heartbeat live outside both repositories and are P-scoped; stale historical `PRYSM_AUTORUN_STATE.json`, Production Closure state, old report-improvement tranches, or unrelated PDV state must not route the active P#.
