...
Date: 2026-09-09
Status: PREPARED — MODEL-BACKED EXECUTION NOT AUTHORIZED
...
Once Chris separately and explicitly authorizes the model-backed run, the authorized execution is one new clean Narrative v2 TBK Writer/Judge release-candidate orchestration against the frozen governed TBK WriterInput already stored in the repository fixture.
...
Maximum automatically authorized model/provider calls for this run after explicit Chris authorization: **4**.

If Judge 2 still returns `REVISE`, the orchestration must stop at governed human review. Writer/Judge Pass 3 is **not** included in this prepared authorization boundary and requires a separate explicit human authorization.

The absolute six-call ceiling remains unchanged but is not permission to use calls 5-6.

## Explicitly prohibited

This prepared boundary does not authorize:
- any Writer/Judge/model/provider call before Chris explicitly authorizes the run;
...
## Required pre-execution checks after authorization
...
If any check fails, stop before a model/provider call.
...
Exact next action: Chris must explicitly authorize this prepared one-run model-backed boundary before any Writer/Judge/model/provider execution occurs.
